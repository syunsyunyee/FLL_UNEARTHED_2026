...
def go_to_statue_rebuild():
        curve(700, 700, -90, 0) #90도 회전하여 뒤로 가는 방향 맞추기
        move(-100)    #뒤로 100(mm)이동하여 조각상 재건 방향으로 이동
        curve(700, 700, 90, 0)  #90도 회전하여 조각상 재건 하러 방향 맞추기
        move(160)     #앞으로 이동하여 팔을 넣을 위치 맞추기
        curve(700, 700, 46, 0)  #41도 회전하여 조각상 재건과 방향 맞추기
        f_motor_run(700, 110)   #오른팔을 내려서 팔이 들어갈 수 있도록 하기
def statue_rebuild():
        move(200)     #조각상에 팔 넣기
        f_m.run(-10)            #왼쪽 팔 드는방향으로 동작 시작하기
        wait(300)               #좀 이따 돌아야 하므로 0.15초 기다리기
        curve(300, 300, -10, 0) #왼쪽으로 10도 돌아 조각상 재건 해결 안정성 높이기
